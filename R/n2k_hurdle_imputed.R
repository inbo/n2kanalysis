...
setMethod(
  ...
  definition = function(
    ...
    validObject(count)
    status <- ifelse(
      all(c(status(presence), status(count)) == "converged"),
      "new",
      "waiting"
    )
    assert_that(
      ...
      gsub("^inla binomial: ", "", get_model_type(presence)) ==
        gsub(
          "^inla zeroinflated(nbinomial|poisson)0: ",
          "",
    ...
    analysis_relation <- data.frame(
      ...
      parentstatus_fingerprint = c(
        get_status_fingerprint(presence),
        get_status_fingerprint(count)
      ),
    ...
    new(
      ...
      AnalysisMetadata = data.frame(
        ...
        seed = presence@AnalysisMetadata$seed,
        status = status,
      ...
      Presence = presence@RawImputed,
      Count = count@RawImputed,
      Hurdle = NULL
    )
  }
)
