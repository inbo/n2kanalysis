...
setValidity(
  ...
  function(object){
    ...
    if (length(object@AnalysisFormula) != nrow(object@AnalysisMetadata)) {
      stop("Number of 'AnalysisFormula' not equal to number of 'AnalysisMetadata'")
    }
    if(class(object@AnalysisMetadata$Formula) == "character") {
      ...
      formula.list <- lapply(levels(object@AnalysisMetadata$Formula), as.formula)
      if (!isTRUE(all.equal(
    ...
    if (any(object@AnalysisMetadata$FirstImportedYear > object@AnalysisMetadata$LastImportedYear)) {
    ...
    if (any(object@AnalysisMetadata$LastAnalysedYear > object@AnalysisMetadata$LastImportedYear)) {
    ...
    if (any(object@AnalysisMetadata$Duration > object@AnalysisMetadata$LastImportedYear - object@AnalysisMetadata$FirstImportedYear + 1)) {
      stop("Some Duration longer than the interval from FirstImportedYear to LastImportedYear")
    }
    
    if (any(object@AnalysisMetadata$LastAnalysedYear < object@AnalysisMetadata$FirstImportedYear + object@AnalysisMetadata$Duration - 1)) {
    ...
    if (!all(object@AnalysisRelation$Analysis %in% object@AnalysisMetadata$FileFingerprint)) {
      stop("Some Analysis in 'AnalysisRelation' slot have no matching FileFingerprint in 'Analysis' slot")
    }
