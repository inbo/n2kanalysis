...
test_that(
  paste(
    "n2kInla with numeric-numeric interaction and two random effects of which",
    "one replicated"
  ),
  {
    dataset <- test_data()
    ...
    analysis <- n2k_inla(
      formula =
        "Count ~ C * D + f(E, model = \"rw1\", replicate = as.integer(A))",
      result_datasource_id = this_result_datasource_id,
      scheme_id = this_scheme_id, species_group_id = this_species_group_id,
      location_group_id = this_location_group_id, family = "nbinomial",
      model_type = this_model_type,
      ...
      param@ParameterEstimate %>%
        inner_join(
          random %>%
            mutate(
              finger2 = ifelse(
                is.na(.data$finger2), .data$finger, .data$finger2
              )
            ),
...
test_that(
  "imputation and aggregation",
  {
    ...
    aggregation <- n2k_aggregate(
      ...
      last_analyses_year = this_last_analysed_year, duration = this_duration,
      parent = get_file_fingerprint(imputation), analysis_date = Sys.time()
    )
    ...
    mi <- n2k_model_imputed(
      ...
      location_group_id = this_location_group_id, model_type = this_model_type,
      first_imported_year = this_first_imported_year,
      analysis_date = Sys.time(), last_imported_year = this_last_imported_year,
      formula = "~ A", last_analyses_year = this_last_analysed_year,
      duration = this_duration, parent = get_file_fingerprint(aggregation)
    )
