test_that(
  paste(
    "n2kInla with categorical and numeric fixed effect without random effect"
  ),
  {
    dataset <- test_data()
...
test_that(
  paste(
    "n2kInla with single random effect, categorical-categorical interaction",
    "and categorical numeric interaction"
  ),
  {
    dataset <- test_data()
    this_analysis_date <- as.POSIXct("2015-01-01 04:05:06.12", tz = "UTC")
...
test_that("imputation and aggregation", {
  ...
  imputation <- n2k_inla(
    ...
    last_imported_year = this_last_imported_year,
    family = "poisson",
    last_analyses_year = this_last_analysed_year,
    duration = this_duration,
